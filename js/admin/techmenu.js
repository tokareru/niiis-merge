...
$( function() {
		...
		function FillMenu(obj)
		{
			for (var i = 0; i < obj["1_layout"].length; i++)
			{
				$("#tabs ul li a")[i].innerText = obj["1_layout"][i].name;
				for (let j = 0; j < obj["2_layout"].length; j++)
				{
					let index1 = obj["1_layout"].length+j+(obj["2_layout"].length*i); //3 + 0 + (9*1)
					let f = $("#tabs ul li a")[index1].hash.substr(6,1);
					let s = $("#tabs ul li a")[index1].hash.substr(8,1);
					if (obj["1_layout"][i].id == f && obj["2_layout"][j].id == s)
					{
						$("#tabs ul li a")[index1].innerText = obj["2_layout"][j].name;
